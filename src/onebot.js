...
export class OneBotClient {
  constructor({ wsUrl, httpUrl, accessToken = '', httpToken = '' }, logger) {
    this.wsUrl = wsUrl;
    this.httpUrl = httpUrl;
    this.accessToken = accessToken;
    this.httpToken = httpToken || accessToken;  // HTTP 可单独用 token，缺省同 accessToken
    this.log = logger;
    this.selfId = null;   // 登录后通过 get_login_info 填充
    this.ws = null;
    this.onEvent = () => {};
    this.onConnect = () => {};
    this.onClose = () => {};
  ...
  async getSelfId() {
    try {
      const d = await this.callApi('get_login_info', {}, 10000);
      this.selfId = d?.user_id ?? null;
      this.log.info(`[onebot] 自身 QQ: ${this.selfId}`);
    } catch { /* 忽略，后续事件 self_id 仍可用 */ }
    return this.selfId;
  }
