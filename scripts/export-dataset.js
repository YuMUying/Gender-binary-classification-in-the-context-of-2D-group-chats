...
// 群隔离验证：--val-groups 0,xxx → 这些群标注的用户整体进验证集（跨群泛化测试）
...
function rowsForUser(userId, cap = Infinity, peerFilter = null) {
  let rows = db.prepare(`
    ...
    FROM messages m WHERE user_id=? AND scene IN ('group','private') AND COALESCE(TRIM(m.text),'') != '' AND m.text NOT LIKE '[图片%' AND m.text NOT LIKE '[动画表情%' AND m.text NOT LIKE '[表情%' AND m.text NOT LIKE '[合并转发%' AND m.text NOT LIKE '[语音%' AND m.text NOT LIKE '[文件%' AND m.text NOT LIKE '[视频%' AND m.text NOT LIKE '[JSON%' ORDER BY time ASC`).all(userId);
...
function labeledUsers(confs) {
  ...
  return db.prepare(`
    SELECT l.user_id, l.gender, l.label_group, COUNT(m.id) c,
           SUM(CASE WHEN LENGTH(m.text) >= 4 AND m.text NOT LIKE '[图片%' AND m.text NOT LIKE '[动画表情%' AND m.text NOT LIKE '[表情%' AND m.text NOT LIKE '[合并转发%' AND m.text NOT LIKE '[语音%' AND m.text NOT LIKE '[文件%' AND m.text NOT LIKE '[视频%' AND m.text NOT LIKE '[JSON%' THEN 1 ELSE 0 END) eff
  ...
  if (mode === 'train') {
    ...
    const rows = db.prepare(`
      ...
      WHERE scene IN ('group','private')
        ...
        AND m.text NOT LIKE '[图片%' AND m.text NOT LIKE '[动画表情%' AND m.text NOT LIKE '[表情%' AND m.text NOT LIKE '[合并转发%' AND m.text NOT LIKE '[语音%' AND m.text NOT LIKE '[文件%' AND m.text NOT LIKE '[视频%' AND m.text NOT LIKE '[JSON%'
