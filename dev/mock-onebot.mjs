...
const users = [
  { user_id: 0, nickname: '猛男一号', card: '' },
  { user_id: 0, nickname: '萌妹一号', card: '小仙女' },
  { user_id: 0, nickname: '路人甲', card: null },
];
const HISTORY = [];
...
wss.on('connection', (ws) => {
  ...
  setTimeout(() => ws.send(JSON.stringify({
    ...
    self_id: 10001, message_id: 888001, group_id: GROUP_ID, user_id: 0,
    message: [{ type: 'text', data: { text: '实时消息：蹲蹲，有瓜吗' } }],
    sender: { user_id: 0, nickname: '萌妹一号', card: '小仙女', role: 'member' },
  })), 300);
  setTimeout(() => ws.send(JSON.stringify({
    post_type: 'message', message_type: 'group', sub_type: 'normal', time: Math.floor(Date.now() / 1000),
    self_id: 10001, message_id: 888002, group_id: GROUP_ID, user_id: 0,
    message: [
      { type: 'text', data: { text: '实时消息：兄弟萌早' } },
      { type: 'image', data: { url: 'http://127.0.0.1:3000/img/1.png', file: '2.png' } },
    ],
    sender: { user_id: 0, nickname: '猛男一号', card: '', role: 'member' },
  })), 500);
});
