// 代理重取 versions.json 找 9.9.32 系列
...
const raw = await rawGet('https://raw.githubusercontent.com/PRO-2684/qqnt-version-history/main/versions.json');
let body = raw.slice(raw.indexOf('\r\n\r\n') + 4);
if (/gzip/i.test(raw.slice(0, raw.indexOf('\r\n\r\n')).toString())) body = zlib.gunzipSync(body);
const j = JSON.parse(body.toString('utf8'));
for (const [k, v] of Object.entries(j)) {
  if (k.includes('9.9.32') || k.includes('9.9.33')) {
    console.log('=====', k, '→', v.version ?? '');
    console.log('  x64:', v.x64?.url ?? '(无)');
  }
}
