# -*- coding: utf-8 -*-
"""extend_erotic_labels.py — 扩展涩情标注（0-3级，逐条保存，供本地微调训练）

- 已标注用户（45人）的 关键词命中消息：全量判定（不再截断60条）
- 随机非命中消息：每人最多80条，总上限2000条，补 0 类
- 输出 research/erotic_labels.jsonl：{id, user_id, text, level}（追加式，可断点续跑）
用法: $env:RELAY_KEY='sk-...'; python research/extend_erotic_labels.py
"""
import json
import os
import random
...
def main():
    ...
    by_user = {}
    for r in rows:
        if r['user_id'] in labeled:
            by_user.setdefault(r['user_id'], []).append((r['id'], r['text']))

    hits_all, non_hits = [], []
    for uid, ms in by_user.items():
        for mid, t in ms:
            if KW.search(t):
                hits_all.append((mid, uid, t))
            else:
                non_hits.append((mid, uid, t))
    random.seed(42)
    random.shuffle(non_hits)
    # 非命中抽样：每人最多80，总上限2000
    per_user = {}
    non_sample = []
    for mid, uid, t in non_hits:
        if per_user.get(uid, 0) >= 80 or len(non_sample) >= 2000:
            continue
        per_user[uid] = per_user.get(uid, 0) + 1
        non_sample.append((mid, uid, t))
    print(f'命中消息: {len(hits_all)} | 非命中抽样: {len(non_sample)} | 合计: {len(hits_all) + len(non_sample)}')
    ...
    todo = [(mid, uid, t) for mid, uid, t in hits_all + non_sample if mid not in done_ids]
    print(f'待判定: {len(todo)}（已完成 {len(done_ids)}）')

    def work(item):
        mid, uid, t = item
        return mid, uid, t, judge_batch([(0, t)])
    ...
    print(f'\n[完成] 累计 {len(all_labels)} 条: ' + dict(Counter(all_labels)).__str__())
