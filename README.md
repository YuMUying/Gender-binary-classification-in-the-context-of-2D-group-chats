...
一项针对中文兴趣社群性别风格推断的研究工件仓库：**模型权重 + 评测代码 + 脱敏样例数据 + 完整的负结果实验记录**。方法与数据构建过程见论文（预印本）；**本仓库不包含语料采集器代码，也不对外提供任何采集工具**。

> **隐私与使用声明**：完整语料（302 万条消息）与标注账本**永不公开**；生产模型权重通过 [Releases](../../releases/tag/v1.0.0) 公开（仅供学术复现，不得用于对真实个体做身份或倾向判定）。**脱敏样例数据集**见 `data-sample/`（说话人以随机别名呈现、日期粗化到日、文本经规则清洗与人工审计；清洗规程与标注准则随样例发布）。历史版本中的哈希别名映射文件已移除（截断哈希不具备不可逆性）；实验记录中的 `U###`/`G###` 仅作匿名代号。
